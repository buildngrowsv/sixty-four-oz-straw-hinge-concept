# 64 oz insulated bottle · gravity-hinged straw concept

Static concept package: interactive 2D comparison (cap flex vs rigid straw vs mid-hinge), inline SVG mechanisms diagram (large silicone sock on cut rigid stubs), reference PNG board, and design notes for short-span silicone joints.

## View locally

Open `SixtyFourOunceFlexibleJointStrawBottleConceptAnimation.html` in a browser (double-click or “Open with”). Assets use relative paths; keep this folder together.

## Publication (Pubroot)

Published on Pubroot after peer review:

- **Article:** [pubroot.com/2026-008/article/](https://pubroot.com/2026-008/article/)
- **Submission issue (history):** [pubroot-website#8](https://github.com/buildngrowsv/pubroot-website/issues/8)

A **three-panel screenshot** from the interactive demo is in `figures/straw-demo-three-panel-bottles.png` (same asset as on the article page). It is captured at a **strong drink tilt** (~84°) via `SixtyFourOunceFlexibleJointStrawBottleConceptAnimation.html?tilt=84&fill=66` so the gravity-hinged straws read clearly against the lid-rigid straw.

## Repository

[https://github.com/buildngrowsv/sixty-four-oz-straw-hinge-concept](https://github.com/buildngrowsv/sixty-four-oz-straw-hinge-concept)

## License

Concept / educational visuals. Confirm licensing with the project owner before commercial reuse of artwork.

## Syncing changes

Edits are often authored under `Personal Agent/.../SixtyFourOunceFlexibleJointStrawBottleConceptAnimationDemos/`. Copy that folder here and commit when the demo changes; bump the commit SHA in any open Pubroot submission if reviewers need a new pin.
